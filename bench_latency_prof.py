...
PROF = 1 # or 0 to disable profile
...
def latency_test_run_once(
    is_warm_up, tp_rank, run_name, model_runner, rank_print, reqs, batch_size, input_len, output_len
):
    ...
    # Prefill
    if False and PROF and not is_warm_up and tp_rank == 0:
    ...
    # Decode
    decode_latencies = []
    if PROF and not is_warm_up and tp_rank == 0:
...
def latency_test(
    ...
    for bs, il, ol in itertools.product(
        ...
        ret = latency_test_run_once(
            False, tp_rank, bench_args.run_name, model_runner, rank_print, reqs, bs, il, ol
        )
        if ret is not None:
