...
do
        ...
        echo Batch_Size=$BS
        python /app/vllm/benchmarks/benchmark_latency.py --model $MODEL_PATH --input-len $IL --output-len $OL --batch-size $BS --tensor-parallel-size $TP --dtype float16 --worker-use-ray 2>&1 | tee latency_llama3_base-$BS-$IL-$OL.log
done
